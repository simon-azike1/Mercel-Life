 ...
 const  PortfolioSection = forwardRef ((props, ref) =>  {
     ...
     initial={{ opacity: 0, y: 40 }}
      ...
      viewport={{ once: true }}
       transition={{ duration: 0.6, delay: 0.1 }}
        >
       <section ref={ref} className="py-20  bg-gray-300"  >
    ...
    A selection of projects that showcase my design process, creative thinking, and problem-solving approach.
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project) => (
            <Card
              key={project.id}
              className="group overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border-none"
            >
  ...
  <div className="mb-2">
    <Link to={project.link}>
      <Badge
        variant="secondary"
        className="text-xs bg-purple-100 text-purple-700 hover:bg-purple-200 hover:text-purple-900 transition-colors cursor-pointer"
      >
        {project.category}
      </Badge>
    </Link>
